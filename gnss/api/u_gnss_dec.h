 ...
 *                                never be NULL;if the protocol
 *                                type is set to
 *                                #U_GNSS_PROTOCOL_UNKNOWN and the
 *                                callback is able to decode the
 ...
 * the returned structure will be non-zero but the protocol type
 * and a message ID may still have been decoded (if the id.type
 * field of the returned structure is not #U_GNSS_PROTOCOL_UNKNOWN).
 *
 ...
 *
 * @param[in] pDec the pointer returned by pUGnssDecAlloc();
 *                 may be NULL.
 */
