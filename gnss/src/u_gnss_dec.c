 ...
 * bit-fields and enums properly and document them all well,
 ...
 * would be named ubxXxxYyy() and must follow the function signature
 * of #uGnssDecKnownFunction_t.
 *
 ...
 *
 * 5. Add at least one test vector for the function to the
 * gTestDataKnownSet array in u_gnss_dec_test.c, using the pattern
 * of gUbxNavPvt as an example.
 *
            ...
            if ((*pBufferUint8 == 0xB5) && (size >= 1) && (*(pBufferUint8 + 1) == 0x62)) {
                ...
                if ((size >= 1 /* D3 */ + 2 /* length */) &&
                    ...
                    if (size >= 1 /* D3 */ + 2 /* length */ + 2 /* ID */) {
                        ...
                        if (size >= y /* length includes the message ID */ + 1 /* D3 */ + 2 /* length */) {
