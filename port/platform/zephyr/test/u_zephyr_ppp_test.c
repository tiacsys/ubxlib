...
#ifndef U_ZEPHYR_PPP_TEST_RECEIVE_TASK_STACK_SIZE_BYTES
/** The stack size to use for the asynchronous receive task.
 */
# define U_ZEPHYR_PPP_TEST_RECEIVE_TASK_STACK_SIZE_BYTES 2560
#endif

#ifndef U_ZEPHYR_PPP_TEST_RECEIVE_TASK_PRIORITY
/** The priority to use for the asynchronous receive task.
 */
# define U_ZEPHYR_PPP_TEST_RECEIVE_TASK_PRIORITY (U_CFG_OS_PRIORITY_MIN + 5)
#endif

#ifndef U_ZEPHYR_PPP_TEST_RECEIVE_TASK_RELAX_MS
/** How long the receive task should relax for between receive
 * attempts.
 */
# define U_ZEPHYR_PPP_TEST_RECEIVE_TASK_RELAX_MS 10
#endif

#ifndef U_ZEPHYR_PPP_TEST_RECEIVE_TASK_EXIT_MS
/** How long to allow for the the receive task to exit;
 * should be quite a lot longer than
 * #U_ZEPHYR_PPP_TEST_RECEIVE_TASK_EXIT_MS.
 */
# define U_ZEPHYR_PPP_TEST_RECEIVE_TASK_EXIT_MS 100
#endif
...
static uZephyrPppSockTestConfig_t gTestConfig = {.taskHandle = NULL};
...
{
    int32_t sizeBytes;
    uZephyrPppSockTestConfig_t *pTestConfig = (uZephyrPppSockTestConfig_t *) pParameter;
    ...
    do {
        sizeBytes = zsock_recv(pTestConfig->sock,
                               pTestConfig->pBuffer +
        ...
        if (sizeBytes > 0) {
            ...
            uPortTaskBlock(U_ZEPHYR_PPP_TEST_RECEIVE_TASK_RELAX_MS);
        }
...
static void osCleanup()
{
    if (gTestConfig.taskHandle != NULL) {
        ...
        uPortTaskBlock(U_ZEPHYR_PPP_TEST_RECEIVE_TASK_EXIT_MS);
...
U_PORT_TEST_FUNCTION("[zephyrSock]", "zephyrSockTcp")
{
    ...
    for (uNetworkTestList_t *pTmp = pList; pTmp != NULL; pTmp = pTmp->pNext) {
        ...
        if ((pModule == NULL) ||
            ...
            zsock_inet_pton(AF_INET, hostIp, &destinationAddress.sin_addr);
            ...
            x = zsock_fcntl(sock, F_GETFL, 0);
            U_PORT_TEST_ASSERT(x >= 0);
            x &= ~O_NONBLOCK;
            U_PORT_TEST_ASSERT(zsock_fcntl(sock, F_SETFL, x) == 0);
            ...
            U_PORT_TEST_ASSERT(uPortTaskCreate(rxTask, "rxTask",
                                               U_ZEPHYR_PPP_TEST_RECEIVE_TASK_STACK_SIZE_BYTES,
                                               (void *) &gTestConfig,
                                               U_ZEPHYR_PPP_TEST_RECEIVE_TASK_PRIORITY,
